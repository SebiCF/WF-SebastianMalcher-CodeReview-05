...
function createFavorites(pageNumberFavorites, user_dataFavorites) {
    ...
    for (i = firstProfile; i < pageNumberFavorites; i++) {
        ...
        class="mx-auto d-block bg-dark" src='${user_dataFavorites[i].image}'><div style="height:140px" class="rounded-bottom mb-5" id="userDesc">Name: ${user_dataFavorites[i].name}&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspGender: ${user_dataFavorites[i].gender}&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspAge: ${user_dataFavorites[i].age}&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspProgress: <div style="float: right; width: 34px;border: 1px solid black;margin-left: -7%;"><div style="width: ${parseInt(user_dataFavorites[i].likeProgress)*20}%;background-color: green;justify-content: center;">${user_dataFavorites[i].likeProgress}</div></div><br>Hobbies:&nbsp&nbsp&nbsp${user_dataFavorites[i].hobbies}<br>${user_dataFavorites[i].quote}<br></div>`)
            .css({
                'height': '250px',
